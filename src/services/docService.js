...
const FILE_PATH = path.join(process.cwd(), 'files');
...
async function genContent(keywords, totalModulesStr, module, index) {
  ...
  // 当存在子模块时
  if (module.hasOwnProperty('subModules') && Array.isArray(module['subModules']) && module['subModules'].length > 0) {
    let parallelGroup = [];

    // 0 生成概述
    parallelGroup.push(agentService.moduleContentAgent(keywords, totalModulesStr, module.moduleName));

    // 1 生成功能组成
    parallelGroup.push(genModuleComposition(module));

    // 2 生成业务流程
    parallelGroup.push(agentService.flowChartAgent(keywords, totalModulesStr, module.moduleName));

    // 3 生成内部交互关系
    parallelGroup.push(genInternalInteraction(keywords, totalModulesStr, module));

    // 4 生成外部交互关系
    parallelGroup.push(genExternalInteraction(keywords, totalModulesStr, module));

    // 5 递归生成子模块的内容
    for (let i = 0; i < module['subModules'].length; i++) {
      let subModule = module['subModules'][i];
      parallelGroup.push(genContent(keywords, totalModulesStr, subModule, index + 1));
    }
    ...
    let functionDetail = await agentService.functionDetailAgent(keywords, totalModulesStr, module.moduleName);
...
async function genInternalInteraction(background, totalModulesStr, module) {
  let table = await agentService.internalInteractionTableAgent(background, totalModulesStr, module.moduleName);
  ...
  let content = commonUtils.replaceKeywords(templates.interaction, replacements);
...
async function genExternalInteraction(background, totalModulesStr, module) {
  let table = await agentService.externalInteractionTableAgent(background, totalModulesStr, module.moduleName);
